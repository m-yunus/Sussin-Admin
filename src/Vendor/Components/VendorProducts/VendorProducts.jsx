...
const VendorProducts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${BaseUrl}/api/product/create`, formData, {
        headers,
      });
  ...
  const fetchcategory = async () => {
    ...
    try {
      const res = await axios.get(`${BaseUrl}/api/product/get-all-categories`, {
        headers,
      });
      ...
      <div className="create-product">
        <div>
          <form onSubmit={handleSubmit}>
            <div className=" flex-wrap create-top-section flex  justify-between  ">
              <div className="flex flex-col  ">
                <h1>Product slug</h1>
                <span className="flex gap-2 my-2 items-center">
                  <p>products/</p>{" "}
                  <input
                    className="w-60 slug-input  h-8 text-sm "
            ...
            <div className="mt-2 flex justify-between vndr-btm ">
              <div className=" w-[243px]  prdct-imgs ">
                  ...
                  <div>
                    <div className="flex h-8 w-full  justify-between">
                      ...
                      <label
                        ...
                        className="pointer text-xs flex items-center bg-green-600 text-white hover:bg-green-800 justify-center w-1/2 "
                      >
                        Upload File
                      </label>

                      <button className="pointer text-xs flex items-center bg-green-600 text-white hover:bg-green-800 justify-center w-1/2 ms-2">
                        Browse images
                      </button>
                    </div>
                  </div>
                ...
                <div className="flex justify-between mt-8">
                  <button className="prvw-btn bg-blue-600 text-white   py-2 h-10 w-24 rounded   hover:bg-blue-700 text-sm">
                    Preview
                  </button>
                  <button
                    type="submit"
                    className="create-btn bg-blue-600 text-white  py-2 h-10 w-20 rounded   hover:bg-blue-700 text-sm"
                  >
                    Create Product
                  </button>
                </div>
              </div>
              <div className=" ms-6 prdct-dscrptn">
                <h1>Product Description</h1>
                <input
                  className="product-description-vendor border mt-2 border-gray-400 h-[305px] w-[680px]"
                  type="text"
                  name="description"
